...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
		...
		treegrid = $('#treegrid').treegrid({
			url : '${pageContext.request.contextPath}/admin/menu/treegrid.do',
			toolbar : [ {
				text : '展开',
				iconCls : 'icon-redo',
				handler : function() {
	...
	function add() {
		var p = parent.js.dialog({
			title : '新增菜单',
			href : '${pageContext.request.contextPath}/admin/menu/menuAdd.do',
			width : 500,
			height : 220,
			buttons : [ {
				text : '新增',
				handler : function() {
					var f = p.find('form');
					f.form('submit', {
						url : '${pageContext.request.contextPath}/admin/menu/add.do',
						success : function(d) {
	...
	function edit() {
		...
		if (node) {
			var p = parent.js.dialog({
				title : '修改菜单',
				href : '${pageContext.request.contextPath}/admin/menu/menuEdit.do',
				width : 500,
				height : 220,
				buttons : [ {
					text : '修改',
					handler : function() {
						var f = p.find('form');
						f.form('submit', {
							url : '${pageContext.request.contextPath}/admin/menu/edit.do',
							success : function(d) {
								var json = $.parseJSON(d);
								if (json.success) {
									treegrid.treegrid('reload');
	...
	function remove() {
		...
		if (node) {
			parent.$.messager.confirm('询问', '您确定要删除<' + node.cname + '>？', function(b) {
				if (b) {
					$.ajax({
						url : '${pageContext.request.contextPath}/admin/menu/remove.do',
						data : {
							cid : node.cid
						},
						cache : false,
